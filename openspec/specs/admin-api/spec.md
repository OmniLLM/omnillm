...
The system SHALL expose authenticated raw and aggregate metering views with model, provider, client, API-shape, and time filters, plus a server-sent-event stream of live logs.

#### Scenario: Filtered metering query
- **WHEN** a metering request supplies supported dimension or time filters
