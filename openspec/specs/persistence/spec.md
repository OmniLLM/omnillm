...
The connection SHALL enable write-ahead logging, foreign keys, a 5000 millisecond busy timeout, and one open and idle connection.

#### Scenario: Concurrent write contention
- **WHEN** writes contend for the lock
- **THEN** a writer waits up to the busy timeout rather than failing immediately
...
Provider instances SHALL own credentials, configuration, per-model state/configuration, and model-list caches, and deletion SHALL cascade to those records.
