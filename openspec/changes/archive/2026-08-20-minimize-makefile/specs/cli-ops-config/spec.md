...
The repository Makefile MUST be limited to desktop sidecar orchestration and
MUST NOT wrap OmniLLM lifecycle, generic Go or Bun development, validation,
...
- **WHEN** a contributor invokes a retained Make target
- **THEN** only `build-desktop-sidecar`, `build-desktop`, and `desktop-dev` are available as cross-platform desktop orchestration

#### Scenario: Direct development tools

- **WHEN** a contributor builds, tests, lints, releases, or runs a container outside desktop packaging
- **THEN** current documentation uses the direct Go, Bun, release-script, or Docker command
